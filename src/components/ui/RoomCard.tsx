...
export default function RoomCard({ room, className = "", variant = "full" }: RoomCardProps) {
  return (
    <div className={`group flex flex-col w-full h-full overflow-hidden ${className}`}>
      ...
      {/* Card Body */}
      <div className={`flex flex-col flex-grow ${variant === 'minimal' ? 'pt-4 space-y-2' : 'pt-6 space-y-4'} whitespace-normal px-1`}>
        {/* Room Title */}
        <h2 className={`font-headline-md text-primary ${variant === 'minimal' ? 'text-2xl md:text-3xl' : 'text-2xl md:text-headline-md'}`}>
          {room.name}
        </h2>
